...
public extension Endpoint {
    ...
    /// - returns: A valid `Endpoint.Media`.
    static func media(_ media: SocialGraphKit.Media) -> Endpoint.Group.Media {
    ...
    static func media(_ identifier: String) -> Endpoint.Single<SocialGraphKit.Media.Unit, Swift.Error> {
    ...
    /// - parameter url: A valid `URL`.
    /// - returns: A valid `Endpoint.Single`.
    static func media(at url: URL) -> Endpoint.Single<SocialGraphKit.Media.Unit, Swift.Error> {
...
public extension Endpoint.Group.Media {
    ...
    var summary: Endpoint.Single<SocialGraphKit.Media.Unit, Swift.Error> {
        .init { secret, session in
            Deferred {
                Request.media(self)
                    .info
                    ...
                    .map(\.data)
                    .wrap()
                    .map(SocialGraphKit.Media.Unit.init)
            }
    ...
    var comments: Endpoint.Paginated < SocialGraphKit.Comment.Collection,
        ...
        .init { secret, session, pages in
            ...
            return Pager(pages) {
                Request.media(self)
                    ...
                    .map(\.data)
                    .wrap()
                    .map(SocialGraphKit.Comment.Collection.init)
    ...
    var likers: Endpoint.Paginated < SocialGraphKit.User.Collection,
                                   RankedOffset<String?, String?>,
                                   Swift.Error> {
        .init { secret, session, pages in
            // Persist the rank token.
    ...
    var link: Endpoint.Single<SocialGraphKit.Media.Link, Swift.Error> {
        .init { secret, session in
            Deferred {
                Request.media(self)
                    .permalink
                    ...
                    .map(\.data)
                    .wrap()
                    .map(SocialGraphKit.Media.Link.init)
            }
            .replaceFailingWithError()
        }
    }
    ...
    var viewers: Endpoint.Paginated < SocialGraphKit.User.Collection,
                                    RankedOffset<String?, String?>,
                                    Swift.Error> {
        .init { secret, session, pages in
            // Persist the rank token.
