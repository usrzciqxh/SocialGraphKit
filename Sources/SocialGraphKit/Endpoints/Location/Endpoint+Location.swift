...
public extension Endpoint {
    ...
    static func location(_ identifier: String) -> Endpoint.Single<SocialGraphKit.Location.Unit, Error> {
    ...
    /// - returns: A valid `Endpoint.Single`.
    static func locations(around coordinates: SocialGraphKit.Location.Coordinates,
                          matching query: String? = nil) -> Endpoint.Single<SocialGraphKit.Location.Collection, Error> {
        .init { secret, session in
            Deferred {
                Request.version1
                    ...
                    .map(\.data)
                    .wrap()
                    .map(SocialGraphKit.Location.Collection.init)
            }
            .eraseToAnyPublisher()
        }
    }
...
public extension Endpoint.Group.Location {
    ...
    var summary: Endpoint.Single<SocialGraphKit.Location.Unit, Error> {
        .init { secret, session in
            Deferred {
                Request.location(self)
                    ...
                    .map(\.data)
                    .wrap()
                    .map(SocialGraphKit.Location.Unit.init)
            }
