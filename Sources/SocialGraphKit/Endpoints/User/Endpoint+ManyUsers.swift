...
public extension Endpoint {
    ...
    /// - returns: A valid `Endpoint.ManyUsers`.
    static func users<C: Collection>(_ users: C) -> Group.ManyUsers where C.Element == SocialGraphKit.User {
...
public extension Endpoint.Group.ManyUsers {
    ...
    var friendships: Endpoint.Single<SocialGraphKit.Friendship.Dictionary, Error> {
        .init { secret, session in
            Deferred {
                Request.friendships
                    ...
                    .map(\.data)
                    .wrap()
                    .map(SocialGraphKit.Friendship.Dictionary.init)
            }
