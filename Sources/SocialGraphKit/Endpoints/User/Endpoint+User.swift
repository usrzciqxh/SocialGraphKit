...
public extension Endpoint {
    ...
    static func user(_ user: SocialGraphKit.User) -> Group.User {
    ...
    static func user(_ identifier: String) -> Endpoint.Single<SocialGraphKit.User.Unit, Error> {
    ...
    /// - returns: A valid `Endpoint.Single`.
    static func user(matching username: String) -> Endpoint.Single<SocialGraphKit.User.Unit, Error> {
        .init { secret, session in
            Deferred {
                Request.users
                    ...
                    .map(\.data)
                    .wrap()
                    .map(SocialGraphKit.User.Unit.init)
            }
            .eraseToAnyPublisher()
        }
    }
...
public extension Endpoint.Group.User {
    ...
    var summary: Endpoint.Single<SocialGraphKit.User.Unit, Error> {
        .init { secret, session in
            Deferred {
                Request.user(self)
                    .info
    ...
    var followers: Endpoint.Paginated<SocialGraphKit.User.Collection, RankedOffset<String?, String?>, Error> {
        paginated("followers", matching: nil)
    }

    /// A list of profiles followed by the user.
    var following: Endpoint.Paginated<SocialGraphKit.User.Collection, RankedOffset<String?, String?>, Error> {
        paginated("following", matching: nil)
    }

    /// The current friendship status between the given user and the logged in one.
    var friendship: Endpoint.Single<SocialGraphKit.Friendship, Error> {
        .init { secret, session in
            Deferred {
                Request.friendships
                    .show
                    ...
                    .map(\.data)
                    .wrap()
                    .map(SocialGraphKit.Friendship.init)
            }
            .eraseToAnyPublisher()
        }
    }
    ...
    var posts: Endpoint.Paginated < SocialGraphKit.Media.Collection,
                                  String?,
                                  Error> {
        .init { secret, session, pages in
            Pager(pages) {
                Request.version1.feed
                    ...
                    .map(\.data)
                    .wrap()
                    .map(SocialGraphKit.Media.Collection.init)
    ...
    var similar: Endpoint.Single<SocialGraphKit.User.Collection, Error> {
        .init { secret, session in
            Deferred {
                Request.discover
                    .chaining
                    ...
                    .map(\.data)
                    .wrap()
                    .map(SocialGraphKit.User.Collection.init)
            }
            .replaceFailingWithError()
        }
    }
    ...
    var tags: Endpoint.Paginated < SocialGraphKit.Media.Collection,
    ...
    func followers(matching query: String) -> Endpoint.Paginated<SocialGraphKit.User.Collection, RankedOffset<String?, String?>, Error> {
    ...
    /// - parameter query: A valid `String`.
    /// - returns: A valid `Endpoint.Paginated`.
    func following(matching query: String) -> Endpoint.Paginated<SocialGraphKit.User.Collection, RankedOffset<String?, String?>, Error> {
...
fileprivate extension Endpoint.Group.User {
    ...
    -> Endpoint.Paginated<SocialGraphKit.User.Collection, RankedOffset<String?, String?>, Error> {
        .init { secret, session, pages in
            // Persist the rank token.
            let rank = pages.rank ?? UUID().uuidString
            // Prepare the actual pager.
