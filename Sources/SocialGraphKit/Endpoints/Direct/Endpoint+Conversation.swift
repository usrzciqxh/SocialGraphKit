...
public extension Endpoint.Group.Direct {
    ...
    func conversation(_ identifier: String) -> Endpoint.Single<SocialGraphKit.Conversation.Unit, Error> {
        conversation(identifier).summary
    }
}

extension SocialGraphKit.Request {
...
public extension Endpoint.Group.Direct.Conversation {
    ...
    var messages: Endpoint.Paginated<Conversation.Unit, String?, Error> {
        .init { secret, session, pages in
            Pager(pages) {
                SocialGraphKit.Request.directThread(self)
                    ...
                    .map(\.data)
                    .wrap()
                    .map(SocialGraphKit.Conversation.Unit.init)
    ...
    func send(_ text: String) -> Endpoint.Single<Wrapper, Error> {
        .init { secret, session in
            do {
                ...
                // Prepare the request.
                return SocialGraphKit.Request.directThreads
